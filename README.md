...
# 📈프로젝트 경험
**프로젝트 제목을 눌러 Github에서 자세한 내용 확인 가능**
...
- **목표**: 다양한 컨텐츠 정보(방문, 리뷰, 지리)를 적극 활용
    ...
    - 데이터셋에 맞춰 Fine-tuning된 BERT를 사용 하기 위해 HuggingFace에 올라온 모델 사용
...
- **목표**: 일반화 성능 향상과 과적합 방지
    ...
    - 가짜 음성 탐지 SOTA 모델인 AASIST 사용
    - Test 데이터에만 존재하는 배경 소음을 제거하기 위한 CleanUNet 사용
    - 두 모델을 합쳐 음성 파일을 넣으면 가짜 여부가 나오는 End-to-End로 제공
...
- **목표**: 교통량과 속도를 예측하여 최적의 신호 도출
    ...
    - 세 기능을 합치기 위한 파이프라인을 설계
    - 실세계에 적용하기 위해 VISSIM으로 검증
- **역할**: 팀장 (MLP모델 개발, 유전 알고리즘 설계,  파이프라인 설계)
- **기술**: LSTM, MLP, 유전 알고리즘, Keras Tuner, VISSIM
...
- **목표**: 사용자의 인지 내용을 지식그래프로 표현 및 영상 추천
    ...
    - 영상의 이해한 단어를 지식그래프로 저장
    - 새로운 영상과 이해한 단어와 비교하여 강의 영상 추천
